...
SELECT 
    CASE
        WHEN POSTAL_CODE IS NOT NULL THEN POSTAL_CODE || ' - ' || STREET_ADDRESS || ' - ' || CITY || ' - ' || STATE_PROVINCE || ' - ' || COUNTRY_ID
        WHEN POSTAL_CODE IS NULL THEN '99999' || ' - ' || STREET_ADDRESS || ' - ' || CITY || ' - ' || STATE_PROVINCE || ' - ' || COUNTRY_ID
    END "주소"
